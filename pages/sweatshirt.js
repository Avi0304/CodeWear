...
const SweatShirt = ({ products }) => {
    return (
        <>
            <Head>
                <title>CoderWear.com - sweatshirts</title>
            ...
            <div>
                <section className="text-gray-600 body-font">
                    <div className='flex flex-col mx-14 lg:mx-50 mt-6'>
                        <h1 className='font-semibold text-black m-2 mb-4 text-2xl md:text-4xl text-center sweatshirtitalize'>Explore our sweatshirts  Collection </h1>
                        <p className='text-sm font-medium text-gray-600 dark:text-gray-400 tracking-tighter'>
                            Upgrade your desk setup with the wide selection of sweatshirts available at Codeswear.com. Our sweatshirts are perfect for every workspace—whether you're coding, gaming, or just browsing. Choose from a variety of styles including programming-inspired designs, anime themes, and minimalist patterns. All of our sweatshirts are made with premium materials for smooth tracking, comfort, and long-lasting durability. Shop now and find the perfect sweatshirt to match your vibe!
                        </p>
                    </div>
                    <div className="container px-5 py-10 mx-auto">
                        <div className="flex flex-wrap -m-4 justify-center">

                            {Object.keys(products).length === 0 && <p>Sorry ! All sweatshirts are Out Of Stock, New sweatshirts are Coming Soon </p>}
...
export async function getServerSideProps(context) {
    ...
    for (let item of products) {
        if (item.title in sweatshirt) {
            if (!sweatshirt[item.title].color.includes(item.color) && item.avaiableQty > 0) {
                sweatshirt[item.title].color.push(item.color)
            }
            if (!sweatshirt[item.title].size.includes(item.size) && item.avaiableQty > 0) {
                sweatshirt[item.title].size.push(item.size)
            }
        } else {
            sweatshirt[item.title] = JSON.parse(JSON.stringify(item))
            if (item.avaiableQty > 0) {
                sweatshirt[item.title].color = [item.color],
                    sweatshirt[item.title].size = [item.size]
            }
        }
    }
    ...
    return {
        props: { products: JSON.parse(JSON.stringify(sweatshirt)) }
    }
}
