...
const Hodiees = ({ products }) => {
    return (
        <>
            <Head>
                <title>CoderWear.com - MousePads</title>
            ...
            <div>
                <section className="text-gray-600 body-font">
                    <div className='flex flex-col mx-14 lg:mx-50 mt-6'>
                        <h1 className='font-semibold text-black m-2 mb-4 text-2xl md:text-4xl text-center capitalize'>Explore our MousePads  Collection </h1>
                        <p className='text-sm font-medium text-gray-600 dark:text-gray-400 tracking-tighter'>
                            Upgrade your desk setup with the wide selection of mousepads available at Codeswear.com. Our mousepads are perfect for every workspace—whether you're coding, gaming, or just browsing. Choose from a variety of styles including programming-inspired designs, anime themes, and minimalist patterns. All of our mousepads are made with premium materials for smooth tracking, comfort, and long-lasting durability. Shop now and find the perfect mousepad to match your vibe!
                        </p>
                    </div>
                    <div className="container px-5 py-10 mx-auto">
                        <div className="flex flex-wrap -m-4 justify-center">

                            {Object.keys(products).length === 0 && <p>Sorry ! All MousePads are Out Of Stock, New MousePads are Coming Soon </p>}
...
export async function getServerSideProps(context) {
    ...
    for (let item of products) {
        if (item.title in mousepad) {
            if (!mousepad[item.title].color.includes(item.color) && item.avaiableQty > 0) {
                mousepad[item.title].color.push(item.color)
            }
            if (!mousepad[item.title].size.includes(item.size) && item.avaiableQty > 0) {
